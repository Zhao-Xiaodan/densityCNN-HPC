...
echo "======================================================================="
echo "CALIBRATION DATASET EXPERIMENTAL STUDY - 🐛 DEBUG VERSION"
echo "======================================================================="
...
echo "  ✅ Force enhanced_preprocessing (keep 512x512, no resize)"
echo "  ✅ Force use_enhanced_model (better architecture)"
echo "  ✅ Fixed dilution factor pattern matching"
echo "  ✅ Added prediction range diagnostics"
echo "  ✅ Early termination if R² < 0 after 10 epochs"
echo "======================================================================="

# Create main timestamped output directory
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
MAIN_OUTPUT_DIR="./calibration_experimental_study_DEBUG_${TIMESTAMP}"
...
run_experiment() {
    ...
    echo ""
    echo "####################################################################"
    echo "STARTING EXPERIMENT $EXP_ID: $EXP_NAME (DEBUG MODE)"
    echo "####################################################################"
    ...
    # ✅ FIXED: Build command line arguments with DEBUG flags
...
echo "=== EXPERIMENT $EXP_ID: $EXP_NAME (DEBUG) ==="
...
echo "🐛 DEBUG MODE: Enhanced preprocessing + Enhanced model enabled"
echo "==============================="
...
echo ""
echo "🚀 Starting Calibration Experimental Study (DEBUG): 4 Architecture Variants"
echo ""
...
echo "📊 ARCHITECTURE VARIANTS (Calibration Dataset - DEBUG MODE)"
...
echo ""
echo "======================================================================="
echo "CALIBRATION EXPERIMENTAL STUDY (DEBUG) COMPLETED - 4 EXPERIMENTS"
echo "======================================================================="
...
EXPERIMENTAL_REPORT="$MAIN_OUTPUT_DIR/calibration_experimental_report_DEBUG.txt"

cat > $EXPERIMENTAL_REPORT << EOF
CALIBRATION DATASET EXPERIMENTAL STUDY REPORT (DEBUG VERSION)
==============================================================
Generated: $(date)
Job ID: $PBS_JOBID
Node: $(hostname)
...
🐛 DEBUG FIXES APPLIED:
=======================
✅ Enhanced preprocessing enabled (keeps images at 512x512)
✅ Enhanced model architecture enabled
✅ Fixed dilution factor pattern matching (dilution_50x_ format)
✅ Added prediction range diagnostics during training
✅ Early termination if R² < 0 after 10 epochs
✅ Verbose output layer diagnostics
...
echo "" >> $EXPERIMENTAL_REPORT
echo "COMPARISON TO FAILED EXPERIMENTAL STUDY:" >> $EXPERIMENTAL_REPORT
echo "=========================================" >> $EXPERIMENTAL_REPORT
echo "Original experimental study (20260204_150508):" >> $EXPERIMENTAL_REPORT
echo "  - ALL 4 experiments failed (R² < 0)" >> $EXPERIMENTAL_REPORT
echo "  - Issue 1: Resized images to 224x224 (lost information)" >> $EXPERIMENTAL_REPORT
echo "  - Issue 2: Used standard model instead of enhanced" >> $EXPERIMENTAL_REPORT
echo "" >> $EXPERIMENTAL_REPORT
echo "DEBUG version fixes:" >> $EXPERIMENTAL_REPORT
echo "  - Keeps images at 512x512 (preserves all information)" >> $EXPERIMENTAL_REPORT
echo "  - Uses enhanced model architecture" >> $EXPERIMENTAL_REPORT
echo "  - Fixed dilution factor pattern matching" >> $EXPERIMENTAL_REPORT
echo "  - Expected: R² > 0.95 (based on architecture study)" >> $EXPERIMENTAL_REPORT

echo "📋 Experimental analysis report saved: $EXPERIMENTAL_REPORT"

echo ""
echo "🎉 CALIBRATION EXPERIMENTAL STUDY (DEBUG) COMPLETED!"
echo "============================================"
...
echo ""
echo "✅ Debug fixes applied - compare results with original failed study"
echo "============================================"
